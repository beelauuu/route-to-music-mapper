...
export default function Dashboard() {
  ...
  const processRun = async (activityId: number) => {
    try {
      ...
      // Redirect to the run visualization page
      window.location.href = `/run/${data.activity.id}`;
    } catch (err: any) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-purple-900 to-gray-900">
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="mb-8">
          ...
          <h1 className="text-4xl font-bold text-white mb-2">Your Runs</h1>
        ...
        {activities.length === 0 ? (
          <div className="text-center py-12 bg-white/10 backdrop-blur-sm rounded-lg border border-white/20">
            ...
            <p className="text-gray-400 text-sm">
              Go for a run and sync with Strava, then come back here!
            </p>
          </div>
        ) : (
              ...
              >
                <div className="flex items-start justify-between">
                  ...
                  <button
                    ...
                    className="ml-4 bg-spotify-green hover:bg-spotify-green/90 disabled:bg-gray-600 disabled:cursor-not-allowed text-white font-semibold py-2 px-6 rounded-lg transition-colors"
                  >
                    {processingId === activity.id
                      ? 'Processing...'
                      : 'Map Music'}
